...
public class EnvAnnotationTests {
    ...
    * 이 때, 특정OS 에서 무시되는 사유를 disabledReason에 기술하여
    ...
    * */
    ...
    @Test
    @EnabledOnOs(value = {OS.WINDOWS,OS.LINUX}, disabledReason = "윈도우와 리눅스에서만 테스트 합니다.")
    public void testWindowsAndLinux(){}

    @Test
    @DisabledOnOs(value = OS.WINDOWS, disabledReason = "윈도우 환경에서는 테스트를 무시합니다.")
    public void testDisableWindow(){}

    /*
    *   @EnabledInJre 를 이용하면 특정 JRE 버전에서만 테스트 하는 것도 가능하다.
    *   @DisableOnJre 를 이용하면 특정 JRE 버전에서만 테스트를 Disabled 하는 것도 가능하다.
    *   @EnabledForJreRange 이용하면 min 과 max 속성 사이의 버전에서 테스트 하는 것도 가능하다.
    *   (min만 작성 시 min~최신버전까지, Max만 작성 시 이전 버전부터 max까지만 테스트한다.)
    *   @DisableForJreRange 를 이용하면 특정 범위 상이의 JRE 환경에서의 테스트를 disabled 할 수 있다.
    * */
